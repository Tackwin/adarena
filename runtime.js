...
Module Progressive_Web_App platform layer inserted from C:/Users/Tackwin/Documents/Code/jai/modules/Toolchains/Web/libjs/Progressive_Web_App.js
...
to put files from the server into te OPFS.
...
their own asset system that fetches files more lazily for example. This implementation uses localStorage in a pretty stupid way
...
const pwa_assets_are_outdated = async () => {
    const time = window.localStorage.getItem(pwa_assets_key_build_time);
...
const pwa_assets_download = async () => {
    ...
    await Promise.all(Array.from({length: assets_to_delete_count}).map(async (_, it_index) => {
        ...
        if (old) {
            const handle = await opfs_find_file(old);
            await handle?.remove();
        }
    }));
